...
async function load() {
  loading.value = true
  try {
    const res = await api.resources({ ...query })
...
async function toggleBan(row) {
  ...
  try {
    await ElMessageBox.confirm(
      banning ? `封禁下载「${row.title}」？所有人将无法下载。可随时解封。` : `解封「${row.title}」？`,
      banning ? '封禁资源' : '解封资源',
      { type: 'warning' },
    )
  } catch {
  ...
  banning ? await api.banResource(row.id) : await api.unbanResource(row.id)
...
<template>
  <div>
    <div class="page-toolbar">
      ...
      <el-input v-model="query.q" placeholder="资源名" clearable style="width: 180px" @keyup.enter="search" @clear="search" />
      <el-select v-model="query.layerType" placeholder="层" clearable style="width: 120px" @change="search">
    ...
    <el-table :data="rows" v-loading="loading" @sort-change="onSort" border>
      ...
      <el-table-column prop="layerType" label="层" width="100" />
